...
#pragma warning restore 0169
		...
		public virtual unsafe void SetEmojiData (byte[] p0)
		{
			...
			try {
				...
				if (GetType () == ThresholdType)
					JNIEnv.CallVoidMethod  (Handle, id_setEmojiData_arrayB, __args);
				else
					JNIEnv.CallNonvirtualVoidMethod  (Handle, ThresholdClass, JNIEnv.GetMethodID (ThresholdClass, "setEmojiData", "([B)V"), __args);
			} finally {
				if (p0 != null) {
					JNIEnv.CopyArray (native_p0, p0);
					JNIEnv.DeleteLocalRef (native_p0);
...
#pragma warning restore 0169
		...
		public virtual unsafe void SetEmojiPath (string p0)
		{
			...
			try {
				...
				if (GetType () == ThresholdType)
					JNIEnv.CallVoidMethod  (Handle, id_setEmojiPath_Ljava_lang_String_, __args);
				else
					JNIEnv.CallNonvirtualVoidMethod  (Handle, ThresholdClass, JNIEnv.GetMethodID (ThresholdClass, "setEmojiPath", "(Ljava/lang/String;)V"), __args);
			} finally {
				JNIEnv.DeleteLocalRef (native_p0);
			}
		}
